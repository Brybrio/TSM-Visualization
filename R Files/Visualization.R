...
org <- c("Holbrookia maculata") #Any name
...
ggplot() + 
  ...
  geom_raster(data=subset(Tsmdf,Hour==hr & Month==mnth & Scenario==scn), aes(x = x, y = y, fill = Category) , interpolate = TRUE) + 
  facet_wrap(~Shade )  + 
...
ggplot() + 
  ...
  scale_fill_manual(values = c("green","yellow","orange","red")) +
  geom_raster(data=subset(Tsmdf,Hour==hr & Month==mnth & Shade==shd), aes(x = x, y = y, fill = Category) , interpolate = TRUE) + 
  facet_wrap(~Scenario )  + 
  coord_quickmap(xlim = c(min(Tsmdf$x), max(Tsmdf$x)), ylim = c(min(Tsmdf$y), max(Tsmdf$y)),expand = TRUE) + 
  theme_bw( ) +
